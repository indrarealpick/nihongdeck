...
function applyTheme(t){
  ...
  const meta=document.querySelector('meta[name="theme-color"]'); if(meta) meta.setAttribute('content', State.theme==='dark'?'#18181B':'#FAFAF8');
...
function drawWeekChart(){
  ...
  const barActive=isDark?'#E07A8C':'#D95970';
...
function drawSrs(){
  ...
  cv.width=W*dpr;cv.height=H*dpr; const ctx=cv.getContext('2d'); ctx.scale(dpr,dpr); ctx.clearRect(0,0,W,H);
  const css=getComputedStyle(document.documentElement);
  const bar=(css.getPropertyValue('--primary').trim())||'#D95970';
  const cText=(css.getPropertyValue('--text').trim())||'#111827';
  const cText2=(css.getPropertyValue('--text-2').trim())||'#6B7280';
